(function($) {
	...
	$.fn.jsUgrList = function(f) {
		return this.bind('ready recheck',function() {
			$.post('http://'+server+':'+port+'/all_docs', {
				user: getCookie("username"),
				token: getCookie("token")
			}, function(data) {
				if(data.ok){
